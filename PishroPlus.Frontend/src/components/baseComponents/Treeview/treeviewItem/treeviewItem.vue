<template>
  <div :class="$attrs.class">
    <div
      :class="['tv-item', modelValue.active ? 'tv-item-active' : '']"
      :draggable="modelValue.draggable"
      @dragstart="(e) => onDragStart(modelValue, e)"
      @drop.prevent.stop="onDragFinished(modelValue)"
      @dragenter.prevent
      @dragover.prevent
      :style="`cursor: ${modelValue.draggable ? 'move' : ''};`"
      @click="setActiveItem(modelValue)"
      @contextmenu="onContextMenu($event)"
      @dblclick="modelValue.dblClick"
      :tabindex="0"
    ...
    >
      <i
        @click="modelValue.open = !modelValue.open"
        v-if="modelValue.children?.length > 0"
        class="tv-item-expand-icon"
        >{{ modelValue.open ? '-' : '+' }}</i
      >
      <v-checkbox
        :indeterminate="indeterminate"
        color="primary"
        v-if="modelValue.hasCheckBox"
        :model-value="modelValue.checked"
        @update:modelValue="($event) => update($event, modelValue)"
        class="flex-grow-0"
        density="compact"
        hide-details
      />
      ...
      <span :class="modelValue.active ? 'text-primary' : 'text-grey'">{{ modelValue.title }}</span>
    </div>

    <div class="child-box" v-show="modelValue.children?.length > 0 && modelValue.open">
      <TreeviewItem
        v-for="(it, idx) in modelValue.children"
        :key="idx"
        v-model="modelValue.children[idx]"
        class="child-item"
        @drag-finished="onDragFinished"
        @drag-started="onDragStart"
        @activated="setActiveItem"
        :tabindex="0"
      ...
      >
        <v-checkbox
          ...
          @update:modelValue="($event) => update($event, modelValue.children[idx])"
          class="flex-grow-0"
          density="compact"
          hide-details
        />
