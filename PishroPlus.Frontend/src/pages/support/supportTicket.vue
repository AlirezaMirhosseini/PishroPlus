    ...
    <v-card-title>
      ...
      <v-card-text>
        ...
        >
          ...
          <template v-slot:column_priority="{ entity }">
            <p :class="rowColor(entity.statusCode)">
              {{ entity.priority }}
            </p>
          </template>
...
export default {
    ...
    ticketsDataTable() {
      return this.tickets
        .map((a) => ({
          ...
          statusCode: a.status,
          priority: utils.showTicketPriority(a.priority)
        }))
        .filter(
          (z) =>
  ...
  methods: {
    ...
    rowColor(statusCode) {
      if (statusCode == 0) return 'text-primary'
      return ''
    },
    ...
    goToTicket(value) {
      this.$router.push(`/support/supportTicket/${value.id}/${value.code}`)
    },
    async getAllTickets() {
      await TicketService.getAll().then((resp) => {
        if (resp.isSuccess) {
