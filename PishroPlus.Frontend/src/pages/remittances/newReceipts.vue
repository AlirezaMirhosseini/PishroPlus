<template>
  <v-card>
    <v-form ref="newreceiptFrm" @submit.prevent="saveAdd">
      <v-card-text>
        <v-row no-gutters>
          <v-col class="px-1 d-flex">
            <p class="text-info" style="padding-bottom: 20px">
              {{ $t('receiptType') }} :
              {{ receiptTypes.filter((x) => x.value === receiptType)[0]?.title }}
            </p>
          </v-col>
        </v-row>
        <v-row no-gutters>
          <v-col md="4" lg="3" sm="6" cols="12" class="d-flex px-1">
            <v-text-field
              name="code"
              v-model="receiptEntity.code"
              :disabled="isAutoCode"
              :rules="[
                (v) => {
          ...
          <v-col md="4" lg="3" sm="6" cols="12" class="px-1">
            <date-picker
              :rules="rule"
              v-model="receiptEntity.dateTime"
              :editable="true"
              ref="datePicker"
          ...
          <v-col md="4" lg="3" sm="6" cols="12" class="px-1">
            <v-autocomplete
              hide-details
              ref="warehouse"
              v-model="receiptEntity.selectedWarehouse"
              :loading="false"
        ...
        <v-row no-gutters>
          <v-col md="4" lg="3" sm="6" cols="12" class="px-1 py-4" v-show="this.receiptType != 2">
            <v-autocomplete
              hide-details
              v-model="receiptEntity.selectedPurchaseInvoice"
              @update:search="onPurchaseInvoiceSearch"
              @update:modelValue="getInvoiceData"
              :loading="false"
              no-filter
              :items="purchaseInvoiceItems"
              :rules="[this.receiptType == 1 ? (v) => !!v || $t('enterRequiredField') : true]"
              item-title="title"
              item-value="id"
          ...
          <v-col md="4" lg="3" sm="6" cols="12" class="px-1 py-4" v-show="this.receiptType == 2">
            <v-autocomplete
              hide-details
              v-model="receiptEntity.selectedSalesInvoice"
              @update:search="onSalesInvoiceSearch"
              @update:modelValue="getInvoiceData"
              :loading="false"
              no-filter
              :items="saleInvoiceItems"
              :rules="[this.receiptType == 2 ? (v) => !!v || $t('enterRequiredField') : true]"
              item-title="title"
              item-value="id"
        ...
        <v-slide-y-transition>
          <v-row no-gutters v-show="showMore">
            <v-col md="4" lg="3" sm="6" cols="12" class="px-1 py-4">
              <v-autocomplete
                v-model="receiptEntity.destinationWarehouse"
                :loading="false"
                :rules="[this.receiptType == 4 ? (v) => !!v || $t('enterRequiredField') : true]"
                no-filter
            ...
            <v-col md="4" lg="3" sm="6" cols="12" class="px-1 py-4">
              <accountSelector
                ...
                :persistent="false"
                v-model="receiptEntity.provider"
                :title="$t('provider')"
            ...
            <v-col md="4" lg="3" sm="6" cols="12" class="px-1 py-4">
              <accountSelector
                justDetailedAccount="true"
                :persistent="false"
                v-model="receiptEntity.transportation"
            ...
            <v-col md="4" lg="3" sm="6" cols="12" class="px-1 py-4">
              <accountSelector
                justDetailedAccount="true"
                :persistent="false"
                v-model="receiptEntity.delivery"
                :title="$t('delivery')"
                addNewRoute="/persons/new"
              />
            </v-col>
      ...
      <v-row no-gutters>
        ...
        >
          <template #toolbar>
            <p class="ma-2" style="font-weight: bold">{{ $t('receiptItems') }}</p>
            <v-btn
          ...
          <template v-slot:column_productCode="{ entity, index }">
            ...
            >
              <template v-slot:append-inner v-if="!entity.unit?.isDefault">
                <span style="font-size: 8pt; color: grey"
                  >{{ entity.unit?.conversionFactor }}x</span
                >
              </template>
            </v-text-field>
          </template>
          ...
          <template v-slot:column_desc="{ entity, index }">
            <v-text-field
              ...
              @change="
                (ev) => {
                  tableItems[index].desc = ev.target.value
                  entity.action = entity.id ? 2 : 1
                }
              "
              style="margin: -10px"
  ...
  <v-dialog
    @keyup.enter="selectTypeOfreceipt"
...
export default {
  ...
  watch: {
    ...
    receiptType(val) {
      this.showMore = val == 4
      if (val == 1) {
        if (this.purchaseInvoiceItems?.length > 0) {
          this.receiptEntity.selectedPurchaseInvoice = this.purchaseInvoiceItems[0]?.id
          this.getInvoiceData(this.receiptEntity.selectedPurchaseInvoice.id)
        }
      } else {
        if (this.isNewreceipt) {
          this.receiptEntity.selectedPurchaseInvoice = null
          this.tableItems = []
        }
      }
    }
  ...
  computed: {
    ...
    receiptType() {
      // console.log(this.receiptEntity?.type);
      return this.receiptEntity?.type ?? 1
    },
    formIsValid() {
      return (
        ((!this.isAutoCode && this.receiptEntity.code) || this.isAutoCode) &&
        this.receiptEntity.selectedWarehouse &&
        this.tableItems.filter((x) => x.product && Object.keys(x.product).length > 0).length > 0 &&
        (this.receiptType == 1 ? this.receiptEntity.selectedPurchaseInvoice : true) &&
        (this.receiptType == 2 ? this.receiptEntity.selectedSalesInvoice : true) &&
        (this.receiptType == 4 ? this.receiptEntity.destinationWarehouse : true)
      )
    },
    isNewreceipt() {
      return this.currentActiveTab.path === '/warehouse/receipts/new'
    },
    generatedDescription: {
      get() {
        var warehouseTitle = this.warehouseItems.filter(
          (x) => x.id == this.receiptEntity.selectedWarehouse
        )[0]?.title
        var destinationWarehouseTitle = this.destinationWarehouseItems.filter(
          (x) => x.id == this.receiptEntity.destinationWarehouse
        )[0]?.title
        var salesInvoiceTitle = this.saleInvoiceItems.filter(
          (x) => x.id == this.receiptEntity.selectedSalesInvoice
        )[0]?.code
        var purchaseInvoiceTitle = this.purchaseInvoiceItems.filter(
          (x) => x.id == this.receiptEntity.selectedPurchaseInvoice
        )[0]?.code

        if (!this.receiptEntity.description || this.receiptEntity.description.length < 1) {
          switch (this.receiptType) {
            ...
            default:
              return warehouseTitle ? `${this.$t('inventoryExitRemitter')} (${warehouseTitle})` : ''
          }
        } else {
          return this.receiptEntity.description
        }
      },
      set(val) {
        this.receiptEntity.description = val
      }
    }
  },
  methods: {
    ...
    onWarehouseSearch(ev) {
      ...
      this.warehouseSearchTimeout = setTimeout(() => {
        this.getWarehouseItems(ev)
      }, 800)
    },
    ...
    changeStartDateTime(value) {
      if (!this.receiptEntity.dateTime) {
        let newDate = new Date(value)
        newDate.setFullYear(newDate.getFullYear() + 1)
        this.receiptEntity.dateTime = newDate.toISOString().split('T')[0]
      }
    },
    addNewRow() {
    ...
    async saveAdd(isDraft) {
      ...
      const formValidation = await this.$refs.newreceiptFrm.validate()

      if (!formValidation.valid || (!isDraft && !this.formIsValid)) return
      let request = {
        code: this.receiptEntity.code,
        dateTime: this.receiptEntity.dateTime,
        warehouseId: this.receiptEntity.selectedWarehouse,
        invoiceId:
          this.receiptType != 2
            ? this.receiptEntity.selectedPurchaseInvoice
            : this.receiptEntity.selectedSalesInvoice,
        type: this.receiptEntity.type,
        providerDetailedAccountId: this.receiptEntity.provider?.selectedDetailedAccount.id,
        transportationDetailedAccountId:
          this.receiptEntity.transportation?.selectedDetailedAccount.id,
        deliveryDetailedAccountId: this.receiptEntity.delivery?.selectedDetailedAccount.id,
        destinationWarehouseId: this.receiptEntity.destinationWarehouse,
        description: this.receiptEntity.description,
        title: this.receiptEntity.title,
        items: this.tableItems
          ...
          .map((m) => {
            ...
            if (!this.isNewreceipt) {
              if (m.id) output.id = m.id
              output.action = m.id ? m.action : 1
            }
            return output
          })
      }
      if (this.isNewreceipt) {
        await receiptService.create(request).then((resp) => {
          if (resp.isSuccess) {
            this.setErrorMessage(this.$t('receiptSuccessfullyCreated'), 'success')
            this.closeAndRefreshTab()
            
            // const currentTabIndex = this.tabs.findIndex(
            //   (tab) => tab.path === '/warehouse/receipts/new' || tab.path.startsWith('/warehouse/receipts/')
            // );
            // if (currentTabIndex !== -1) {
            //   this.closeTab(currentTabIndex);
            // }
            
            // const receiptListTabExists = this.refreshTabByName('remittanceOrReceipt');
            // if (!receiptListTabExists) {
            //   this.$router.push('/warehouse/remittance-and-receipt/receipts');
            // }
          } else {
        ...
        request.sourceWarehouseId = this.receiptEntity.selectedWarehouse
        await receiptService.update(request).then((res) => {
          if (res.isSuccess) {
            this.setErrorMessage(this.$t('receiptSuccessfullyUpdated'), 'success')
            this.closeAndRefreshTab()
            
            // const currentTabIndex = this.tabs.findIndex(
            //   (tab) => tab.path.startsWith('/warehouse/receipts/')
            // );
            // if (currentTabIndex !== -1) {
            //   this.closeTab(currentTabIndex);
            // }
    ...
    selectTypeOfreceipt() {
      ...
      this.receiptEntity.type = this.receiptType
    ...
    async getreceiptById() {
      ...
      if (res.isSuccess) {
        this.receiptEntity = {
          ...
          selectedSalesInvoice: res.value.type === 2 ? res.value.invoiceId : null,
          selectedPurchaseInvoice: res.value.type !== 2 ? res.value.invoiceId : null,
        ...
        if (res.value.provider) {
          this.receiptEntity.provider = {
            selectedDetailedAccount: {
              detailedAccountId: res.value.provider.id,
              fName: res.value.provider?.fName || res.value.provider?.name,
              fCode: res.value.provider?.fCode || res.value.provider?.code,
              code: res.value.provider.code,
              name: res.value.provider.name
            }
          }
        }

        if (res.value.transportation) {
          this.receiptEntity.transportation = {
            selectedDetailedAccount: {
              detailedAccountId: res.value.provider.id,
              fName: res.value.provider?.fName || res.value.provider?.name,
              fCode: res.value.provider?.fCode || res.value.provider?.code,
        ...
        if (res.value.delivery) {
          this.receiptEntity.delivery = {
            selectedDetailedAccount: {
              detailedAccountId: res.value.provider.id,
              fName: res.value.provider?.fName || res.value.provider?.name,
              fCode: res.value.provider?.fCode || res.value.provider?.code,
    ...
    async getDefaultWarehouses() {
      settingService.GetWarehouseSetting().then((res) => {
        ...
        this.receiptEntity.selectedWarehouse = res.entities.filter(
  ...
  async created() {
    ...
    if (!this.isNewreceipt) await this.getreceiptById()
    if (this.isNewreceipt) this.showTypeSelectionDialog = true
    if (this.receiptType == 1) {
      this.receiptEntity.selectedPurchaseInvoice = this.purchaseInvoiceItems[0]?.id
    } else {
      // this.receiptEntity.fselectedSalesInvoice = null
      if (this.isNewreceipt) this.tableItems = []
    }
    if (this.receiptEntity.selectedPurchaseInvoice || this.receiptEntity.selectedSalesInvoice)
      this.getInvoiceData(
        this.receiptEntity.selectedPurchaseInvoice || this.receiptEntity.selectedSalesInvoice
      )
